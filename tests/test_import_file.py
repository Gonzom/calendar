...
FILE_NOT_EXIST_INVALID_EXTENSION = r"tests/files_for_import_file_tests/sample.blabla"
FILE_BLABLA_EXTENSION = r"tests/files_for_import_file_tests/sample2.blabla"

FILE_NOT_EXISTS_INVALID_EXTENSION = r"tests/files_for_import_file_tests/sample.blabla"
FILE_EXISTS_INVALID_EXTENSION = r"tests/files_for_import_file_tests/sample2.blabla"
...
def test_is_file_extension_valid(test_file, extension, expected):
    if extension is None:
        ...
        result = import_file._is_file_extension_valid(test_file, extension)
...
@pytest.mark.parametrize("row, expected", TEXT_VALID_TESTS)
def test_is_event_text_valid(row, expected):
    assert import_file._is_event_text_valid(row) == expected


@pytest.mark.parametrize("txt_file, expected", IMPORT_TXT_FILE_TESTS)
def test_get_data_from_txt_file(txt_file, expected):
    assert import_file._get_data_from_txt_file(txt_file) == expected


@pytest.mark.parametrize("ics_file, expected", IMPORT_ICS_FILE_TESTS)
def test_get_data_from_ics_file(ics_file, expected):
    assert import_file._get_data_from_ics_file(ics_file) == expected
...
def test_is_file_valid_to_import(test_file, expected):
    assert import_file.is_file_valid_to_import(test_file) == expected


# @pytest.mark.parametrize("event, cal_content, expected",
#                          save_calendar_content_txt_tests)
# def save_calendar_content_txt(event, cal_content, expected, session):
#     assert _get_data_from_txt_file(event, cal_content, session) == expected
