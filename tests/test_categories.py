...
class TestCategories:
    ...
    def test_get_categories_logic_succeeded(session, user, category):
        assert _get_user_categories(session, category.user_id) == [category]
    ...
    def test_creating_new_category(client, user):
        ...
        assert {
            ...
            ("name", "Foo"),
            ("color", "eecc11"),
        }.issubset(set(response.json()['category'].items()))
    ...
    def test_get_category_bad_request(client):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert TestCategories.ILLEGAL_PARAMS in response.json()["detail"]
    ...
    def test_to_dict(category):
        assert ({columns.name: getattr(category, columns.name)
                 for columns in category.__table__.columns}
                == category.to_dict())
    ...
    def test_validate_request_params(params, expected_result):
        assert _is_query_params_valid(params) == expected_result
    ...
    def test_get_categories_failed(session):
        ...
        assert _get_user_categories(session, 1) == []
