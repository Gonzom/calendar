...
from app.routers.share import accept_invitation
...
async def accept_invitations(request: Request, db: Session = Depends(get_db)
    ...
    See share.accept_invitation() for more information.
    ...
    accept_invitation(invitation, db)
...
def get_invitation_by_id(invitation_id: int, db: Session
    ...
    """
    return db.query(Invitation).filter_by(id=invitation_id).first()
